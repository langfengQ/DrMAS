...
#
# Agent-count scaling variant, VANILLA-GRPO counterpart of run_math_4agent.sh: same 4-agent math
# workflow (Solver Agent -> Critic Agent -> Refiner Agent -> Verifier Agent, looped -- see
# agent_system/agent/orchestra/math/math_orchestra.py), but with the ORIGINAL global (mu, sigma)
# GRPO baseline instead of Dr. MAS's per-agent (mu_k, sigma_k) normalization.
#
# This is the necessary control for the agent-count-scaling question (reviewer: "how well does the
# proposed normalization generalize ... to more agents?"): comparing this script against
# run_math_4agent.sh isolates whether Dr. MAS's improvement over vanilla GRPO (measured at 2 agents
# via run_math_lemma_diagnostics.sh vs. run_math.sh) persists, shrinks, or grows once there are 4
# distinct agent roles competing for the same global baseline.
...
algorithm=grpo
group_size=8
group_by_agent_id=False # vanilla GRPO baseline (global mu, sigma) over 4 agent roles
...
experiment_name="grpo_4agent_${model_name_tag}"
