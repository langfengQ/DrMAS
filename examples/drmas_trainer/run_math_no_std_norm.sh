...
#
# "Dr. MAS without sigma_k division" -- i.e. Dr.GRPO (https://arxiv.org/abs/2503.20783) applied at
# the per-agent level: advantages are centered by each agent's own active-step mean mu_k, but are
# NOT divided by the per-agent std sigma_k. This isolates whether Dr. MAS's gain comes from
# mean-recentering (agent-aware baseline) alone, vs. also needing the std normalization -- and
# sidesteps the sigma_k -> 0 blow-up risk raised for the full (mu_k, sigma_k) variant, exactly as
# Dr.GRPO removes std-normalization to avoid that risk at the global level.
#
# This only differs from run_math_std_tracking.sh / run_math.sh by one flag:
#   algorithm.norm_adv_by_std_in_grpo=False
# (advantage = score - mu_k, instead of (score - mu_k) / (sigma_k + eps))
#
# The same adv_std/<agent_id>/* and adv_norm/<agent_id>/* wandb metrics from
# run_math_std_tracking.sh are still logged here for reference (sigma_k is still computed/reported
# for monitoring purposes, it is simply not used to scale the advantage), so the two runs can be
# directly compared in wandb: same sigma_k trajectories, different training dynamics.
...
algorithm=grpo
group_size=8
group_by_agent_id=True         # enable Dr. MAS-style per-agent grouping (mu_k)
norm_adv_by_std_in_grpo=False  # do NOT divide by sigma_k -- Dr.GRPO-style, mean-only, per agent
...
experiment_name="drmas_nostdnorm_share${model_sharing}_${model_name_tag}"
